...
public class WizardRequest
{
  ...
  public WizardRequest(HttpServletRequest httpServletRequest, List<IWizardPanel> panelList)
  {
    ...
    if(path.length > 2)
    {
      ...
      {
        throw new IllegalArgumentException("\"" + path[2] + "\" is not a valid panel number");
      }
      
      if(panelId_ < 0 || panelId_ >= panelList.size())
      {
        throw new IllegalArgumentException("Panel ID out of range (" + panelId_ + " passed, max=" + panelList.size() + ")");
      }
    }
