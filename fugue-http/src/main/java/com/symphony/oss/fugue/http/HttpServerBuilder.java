...
package com.symphony.oss.fugue.http;
...
public class HttpServerBuilder implements IServletContainer
{
  ...
  {
    resourceHandlers_.add(new FugueResourceHandler(provider));
  ...
  public HttpServerBuilder withServlet(IUrlPathServlet servlet)
  {
    if(servletMap_.containsKey(servlet.getUrlPath()))
      throw new IllegalArgumentException("Path \"" + servlet.getUrlPath() + "\" is already mapped.");
  ...
  public HttpServer build()
  {
    if(httpPort_ == -1 && httpsPort_ == -1)
      throw new IllegalStateException("One of httpPort and httpsPort must be set");
...
//    http_config.setSecurePort(httpsPort_);
    ...
    if(httpsPort_ != -1)
    {
      ...
      if(keyStorePath_ == null || keyStorePassword_ == null)
        throw new IllegalStateException("When httpsPort is set keyStorePath and keyStorePassword must also be set.");
