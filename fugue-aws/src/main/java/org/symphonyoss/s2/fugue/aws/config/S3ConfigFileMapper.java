  ...
  public S3ConfigFileMapper(String variableName, URL s3Url)
  {
    if(s3Url != null)
    {
      ...
      if(host.endsWith(AWS_COM))
      {
        ...
        {
          throw new IllegalStateException("Failed to load config file from " + s3Url + " to " + target);
        }
      }
      else
      {
        throw new IllegalStateException("s3Url is " + s3Url + " a " + AWS_COM + " hostname is expected.");
      }
    }
  }
